		...
		<div id={S.footer}>
			<div className={`container px-md-0 ${S.container}`}>
				<div className="d-lg-flex">
					<div className={S.first}>
							...
							<div className={`${S.accordionWrapper} d-lg-none`}>
								<Accordion>
									<Accordion.Item className={S.accItem} eventKey="0">
										<Accordion.Header className={`${S.accTitle}`}>
											Quick Link
										</Accordion.Header>
										<Accordion.Body className={S.accbody}>
											<ul className="mb-4 p-0">
												{footerMenu.quickLinks.map(
													(item, index) => (
														<li key={index}>
															<Link href={item.link}>
																{item.title}
						...
						<div className={S.address1}>
							...
							<address className={S.mainAddress}>
								12991 Potranco Road Suite 108, San Antonio, TX, 78253,
								United State
							</address>
						</div>
						<div>
							...
							<address className={S.mainAddress}>
								Monday through Friday 8 AM - 5 PM Saturday - Sunday
								Closed
							</address>
						</div>
						<div>
							<div className={S.phone}>
								Phone number:{" "}
								<a href="tel:+(726)888-9024">+(726) 888-9024</a>{" "}
							</div>
						</div>
					</div>
					<div className={`${S.second} d-none d-lg-block`}>
						...
						<ul className="m-0 p-0">
							{footerMenu.quickLinks.map((item, index) => (
								<li key={index}>
					...
					<div className={`${S.third} d-none d-lg-block`}>
						...
						<ul className="m-0 p-0">
							{footerMenu.helpAndSupport.map((item, index) => (
								<li key={index}>
									<Link href={item.link}>{item.title}</Link>
								</li>
							))}
					...
					<div className={S.fourth}>
						<h3 className={S.newsTitle}>Newsletter</h3>
						<div className={S.newsDesc}>
							Let`s us keep you inform about special promotions, office
							events and more
						</div>
						<form action="">
				...
				<p className={S.copyrightText}>
					Copyright © 2021 Pawscare by animalhospital. All Rights Reserved.
				</p>
			</div>
		</div>
	);
