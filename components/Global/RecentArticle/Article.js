        ...
        <section id={S.article}>
            <div className={`container ${S.container}`}>
                <div className={`d-flex flex-column justify-content-center text-center`}>
                    ...
                    <div className={`row g--4 row-cols-1 row-cols-md-2 row-cols-lg-3 gx-4 ${S.row}`}>
                        {
                            data.map((item, index) => <div key={index} className="col">
                                <div className={`${S.single_wrapper} card h-100 border-0`}>
                                    ...
                                    <div className="card-body p-0 m-0 ">
                                        <div className={S.contents}>
                                            ...
                                            <h5 className={S.single_titme}>{item.title}</h5>
                                            <p className={S.short_description}>{item.shortDescription}</p>
                                        </div>
                                    </div>
                                    <div className={`card-footer m-0 border-0 text-start bg-transparent ${S.card_footer}`}>
                                        <button className={S.read_more} onClick={() => router.push("/resources")}>Read More <svg width="15" height="6" viewBox="0 0 15 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M14.018 4.8576L10.8985 0.269362C10.8416 0.185602 10.7691 0.117874 10.6864 0.0712642C10.6037 0.0246544 10.513 0.000373529 10.421 0.000244141H9.18844C9.061 0.000244141 8.99062 0.170097 9.06861 0.287009L11.8134 4.32377H0.301586C0.217892 4.32377 0.149414 4.40319 0.149414 4.50024V5.82377C0.149414 5.92083 0.217892 6.00024 0.301586 6.00024H13.5387C14.0484 6.00024 14.3319 5.32083 14.018 4.8576Z" fill="#103559" />
                                        </svg></button>
                                    </div>
